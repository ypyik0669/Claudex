...
const USER_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass201-data-"));
const GIT_PROJECT_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass201-git-"));
const FAKE_BIN_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass201-bin-"));
const FAKE_CLAUDE = path.join(FAKE_BIN_DIR, "claude.cmd");
const DATA_FILE = path.join(USER_DATA_DIR, "desktop-data.json");
const TARGET_FILE = "pass201-changes.txt";
const RUN_ID = "pass201-git-run";
...
function writeFakeClaude() {
  ...
  fs.writeFileSync(
    ...
    [
      "@echo off",
      "if \"%1\"==\"--version\" (echo claude fake pass201& exit /b 0)",
...
function setupGitProject() {
  ...
  fs.writeFileSync(path.join(GIT_PROJECT_DIR, TARGET_FILE), "pass201 baseline\n", "utf8");
  runGit(["init"]);
  ...
  fs.writeFileSync(path.join(GIT_PROJECT_DIR, TARGET_FILE), "pass201 baseline\npass201 dirty changes evidence\n", "utf8");
}

function writeInitialStore() {
  writeFakeClaude();
  setupGitProject();
  const project = { name: "pass201-git-project", path: GIT_PROJECT_DIR };
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      sessions: [
        {
          id: "pass201-session",
          title: "Pass201 latest git action palette evidence",
      ...
      commandRuns: [
        {
          ...
          code: 128,
          durationMs: 201,
          stdout: "pass201 git stdout before failure",
      ...
      runEvents: [
        {
          ...
          type: "git-command",
          status: "error",
          title: "Git: pass201 stage failed",
          detail: `pass201 latest git action detail · git add missing-${TARGET_FILE}`,
          commandLine: `git add missing-${TARGET_FILE}`,
          cwd: GIT_PROJECT_DIR,
          project,
          sessionId: "pass201-session",
          code: 128,
          durationMs: 201,
...
async function runTest() {
  ...
  if (!win) throw new Error("PASS201_FAILED_NO_WINDOW");
  win.setBounds({ x: 0, y: 0, width: 1500, height: 980 });
  await wait(700);

  assertStep("PASS201_READY", await waitFor(win, "Boolean(document.querySelector('.app-grid') && window.claudexDesktop)", 15000));
  assertStep("PASS201_GIT_STATE_READY_WITHOUT_NOTICE", await waitFor(win, `
    (async function() {
  ...
  assertStep("PASS201_OPEN_PALETTE_LATEST_GIT_ACTION", await openPaletteAndQuery(win, "latest git action pass201"));
  assertStep("PASS201_PALETTE_LATEST_GIT_ACTION_COMMAND", await waitFor(win, `
    (function() {
      const button = [...document.querySelectorAll('.command-modal .command-list button')]
        .find((candidate) => (candidate.getAttribute('data-command-id') || '') === 'git-latest-action:${RUN_ID}');
      ...
      return Boolean(button &&
        ...
        /Git: pass201 stage failed/.test(text) &&
        /git add missing-${TARGET_FILE}/.test(text) &&
        /128/.test(text));
    })();
  `, 8000));

  assertStep("PASS201_CLICK_LATEST_GIT_ACTION_COMMAND", await clickLatestGitActionCommand(win));
  assertStep("PASS201_COMMAND_OPENS_CHANGES_EVIDENCE", await waitFor(win, `
    (function() {
      ...
      return /\\u53d8\\u66f4/.test(active) &&
        /Git: pass201 stage failed/.test(latest) &&
        /pass201 latest git action detail/.test(latest) &&
        /git add missing-${TARGET_FILE}/.test(latest) &&
        /pass201 git stdout before failure/.test(latest) &&
        /pathspec|did not match|fatal/i.test(latest);
    })();
  `, 10000));

  assertStep("PASS201_OPEN_LATEST_GIT_TIMELINE", await win.webContents.executeJavaScript(`
    (function() {
  ...
  `));
  assertStep("PASS201_TIMELINE_FOCUSED_ON_LATEST_GIT_ACTION", await waitFor(win, `
    (function() {
      const active = document.querySelector('.workspace-context-button.active')?.textContent || document.querySelector('.bottom-panel-tabs button.active')?.textContent || '';
      ...
      return Boolean(/\\u8f93\\u51fa|Outputs/i.test(active) &&
        ...
        /Git: pass201 stage failed/.test(text) &&
        /git add missing-${TARGET_FILE}/.test(text) &&
        /pass201 git stdout before failure/.test(text) &&
        /pathspec/.test(text) &&
        panel.querySelector('[data-run-event-type="git-command"]'));
    })();
  `, 10000));

  console.log("PASS201_COMMAND_PALETTE_LATEST_GIT_ACTION_DONE");
  cleanup();
  app.exit(0);
}
...
app.whenReady().then(runTest).catch((error) => {
  console.error("PASS201_FAILED", error?.stack || error);
  Promise.resolve()
    .then(async () => {
      const win = BrowserWindow.getAllWindows()[0];
      if (!win) return;
      ...
      console.error("PASS201_DEBUG", JSON.stringify(debug, null, 2).slice(0, 8000));
    })
    .finally(() => {
      cleanup();
      app.exit(1);
