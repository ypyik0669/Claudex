...
const USER_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass63-data-"));
const FAKE_BIN_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass63-bin-"));
const PROJECT_FIXTURE_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass63-project-"));
...
fs.writeFileSync(path.join(PROJECT_FIXTURE_DIR, "package.json"), JSON.stringify({ name: "pass63" }), "utf8");
writeJson(path.join(USER_DATA_DIR, "desktop-data.json"), {
  ...
  activeProject: { name: "pass63-project", path: PROJECT_FIXTURE_DIR },
  projects: [{ name: "pass63-project", path: PROJECT_FIXTURE_DIR }],
  sessions: [
    {
      id: "default",
      title: "新聊天",
      project: "pass63-project",
...
async function runTest() {
  ...
  if (!win) throw new Error("PASS63_FAILED_NO_WINDOW");
  win.setBounds({ x: 0, y: 0, width: 1480, height: 960 });
  await wait(700);

  assertStep("PASS63_READY", await waitFor(win, "Boolean(document.querySelector('.app-grid') && window.claudexDesktop)", 15000));
  assertStep("PASS63_OPEN_CAPABILITIES", await win.webContents.executeJavaScript(`
    (function() {
  ...
  `));
  assertStep("PASS63_OPEN_MCP_TAB", await waitFor(win, "Boolean(document.querySelector('.plugin-manager-tabs'))", 10000) && await win.webContents.executeJavaScript(`
    (function() {
  ...
  `));
  assertStep("PASS63_MCP_ROW_ACTIONS_VISIBLE", await waitFor(win, `
    (function() {
      const row = [...document.querySelectorAll('.structured-plugin-row')]
        .find((item) => /filesystem/.test(item.textContent || ''));
      if (!row) return false;
      const buttons = [...row.querySelectorAll('.structured-row-actions button')].map((button) => button.textContent || '');
      return buttons.some((text) => /Claude/.test(text)) &&
        buttons.some((text) => /复制/.test(text)) &&
        buttons.some((text) => /记录/.test(text));
    })();
  `, 15000));
  assertStep("PASS63_COPY_MCP_RAW", await win.webContents.executeJavaScript(`
    (function() {
      const row = [...document.querySelectorAll('.structured-plugin-row')]
        .find((item) => /filesystem/.test(item.textContent || ''));
      const button = [...(row?.querySelectorAll('.structured-row-actions button') || [])]
        .find((candidate) => /复制/.test(candidate.textContent || ''));
      if (!button) return false;
      button.click();
      return true;
    })();
  `));
  assertStep("PASS63_COPY_MCP_RAW_FEEDBACK", await waitFor(win, `
    (function() {
      const row = [...document.querySelectorAll('.structured-plugin-row')]
        .find((item) => /filesystem/.test(item.textContent || ''));
      return /已复制/.test(row?.textContent || '');
    })();
  `, 5000));
  ...
  assertStep("PASS63_OPEN_CLAUDE_FROM_MCP_ROW", await win.webContents.executeJavaScript(`
    (function() {
      const row = [...document.querySelectorAll('.structured-plugin-row')]
        .find((item) => /filesystem/.test(item.textContent || ''));
      const button = [...(row?.querySelectorAll('.structured-row-actions button') || [])]
        .find((candidate) => /Claude/.test(candidate.textContent || ''));
      if (!button) return false;
      button.click();
      return true;
    })();
  `));
  assertStep("PASS63_CLAUDE_PANEL_VISIBLE", await waitFor(win, `
  ...
  console.log("PASS63_MCP_ROW_ACTIONS_DONE");
  cleanup();
  app.exit(0);
}

app.whenReady().then(runTest).catch((error) => {
  console.error("PASS63_FAILED", error?.stack || error);
  cleanup();
  app.exit(1);
});

setTimeout(() => {
  console.error("PASS63_TIMEOUT");
  cleanup();
  app.exit(1);
}, 90000);
