...
const USER_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass110-data-"));
const FAKE_BIN_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass110-bin-"));
const PROJECT_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass110-project-"));
const DATA_FILE = path.join(USER_DATA_DIR, "desktop-data.json");
const COMMAND_LOG = path.join(USER_DATA_DIR, "claude-command-log.txt");
const DOCTOR_COUNT_FILE = path.join(USER_DATA_DIR, "pass110-doctor-count.txt");
...
if (args[0] === '--version') out('2.11.0 (Claude Code QA)');
...
else if (args[0] === 'mcp' && args[1] === 'list') out('? pass110-mcp: connected');
  ...
  if (nextDoctorCount === 1) {
    process.stderr.write('pass110 doctor failed\\n');
    process.exit(30);
  }
  out('pass110 doctor ok');
}
else out('pass110 fake claude command: ' + args.join(' '));
`;
...
function writeInitialStore(claudeCommand) {
  ...
  fs.writeFileSync(path.join(PROJECT_DIR, "package.json"), JSON.stringify({ name: "pass110-project" }), "utf8");
  writeJson(DATA_FILE, {
    version: 1,
    settings: {
      provider: "anthropic",
    ...
    activeProject: { name: "pass110-project", path: PROJECT_DIR },
    projects: [{ name: "pass110-project", path: PROJECT_DIR }],
    sessions: [
      {
        id: "default",
        title: "Claude retry recovery",
        project: "pass110-project",
...
async function runTest() {
  ...
  if (!win) throw new Error("PASS110_FAILED_NO_WINDOW");
  win.setBounds({ x: 0, y: 0, width: 1480, height: 960 });
  await wait(700);

  assertStep("PASS110_READY", await waitFor(win, "Boolean(document.querySelector('.app-grid') && window.claudexDesktop)", 15000));
  assertStep("PASS110_OPEN_CLAUDE_TOOL", await openClaudeTool(win));
  assertStep("PASS110_CLAUDE_READY", await waitFor(win, `
  ...
  assertStep("PASS110_RUN_DOCTOR", await runClaudeQuick(win, "doctor"));
  assertStep("PASS110_DOCTOR_FAILURE_RAN", await waitForLogGrowth(/doctor/, beforeFailure, 12000));
  assertStep("PASS110_FAILURE_RETRY_VISIBLE", await waitFor(win, `
    (function() {
      const card = document.querySelector('#claude-tool-detail .command-output-card.error');
      ...
      return Boolean(
        ...
        /doctor/.test(text) &&
        /30/.test(text) &&
        /pass110 doctor failed/.test(text) &&
        retry
      );
    })();
  `, 10000));
  ...
  assertStep("PASS110_CLICK_RETRY", await win.webContents.executeJavaScript(`
    (function() {
      const card = document.querySelector('#claude-tool-detail .command-output-card.error');
      const retry = [...(card?.querySelectorAll('button') || [])]
        .find((button) => /\\u91cd\\u8bd5/.test(button.textContent || ''));
  ...
  `));
  assertStep("PASS110_RETRY_DOCTOR_RAN", await waitForLogGrowth(/doctor/, beforeRetry, 12000));
  assertStep("PASS110_RETRY_RECOVERED", await waitFor(win, `
    Boolean(document.querySelector('#claude-tool-detail .command-output-card.ok') &&
      ...
      /pass110 doctor ok/.test(document.querySelector('#claude-tool-detail .command-output-card.ok')?.textContent || ''))
  `, 15000));

  assertStep("PASS110_OPEN_OUTPUTS_PANEL", await openOutputsPanel(win));
  assertStep("PASS110_BOTTOM_EVIDENCE_VISIBLE", await waitFor(win, `
    Boolean(document.querySelector('.claude-command-evidence-stack') &&
      ...
      /doctor/.test(document.querySelector('.claude-command-evidence-stack')?.textContent || '') &&
      /pass110 doctor ok/.test(document.querySelector('.claude-command-evidence-stack')?.textContent || ''))
  `, 10000));
  assertStep("PASS110_TIMELINE_BOTH_RUNS_VISIBLE", await waitFor(win, `
    (function() {
  ...
  `, 10000));
  assertStep("PASS110_RETRY_PERSISTED", await waitFor(win, `
    (async function() {
      const state = await window.claudexDesktop.getState();
      const runs = state.commandRuns?.filter((run) => run.kind === 'claude' && /doctor/.test(run.command || '')) || [];
      return Boolean(
        runs.length >= 2 &&
        runs.some((run) => run.code === 30 && /pass110 doctor failed/.test(run.stderr || '')) &&
        runs.some((run) => run.code === 0 && /pass110 doctor ok/.test(run.stdout || ''))
      );
    })();
  `, 10000));

  console.log("PASS110_CLAUDE_PANEL_RETRY_RECOVERY_DONE");
  cleanup();
  app.exit(0);
}
...
app.whenReady().then(runTest).catch((error) => {
  console.error("PASS110_FAILED", error?.stack || error);
  cleanup();
  app.exit(1);
});

setTimeout(() => {
  console.error("PASS110_TIMEOUT");
  cleanup();
  app.exit(1);
}, 90000);
