...
const EXE_PATH = path.join(PROJECT_PATH, "release-pass33", "win-unpacked", "Claudex.exe");
const PORT = 12533 + Math.floor(Math.random() * 500);
...
async function main() {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass33-packaged-"));
  ...
  try {
    ...
    assertStep("PACKAGED_PASS33_READY", await waitForEval(cdp, "Boolean(document.querySelector('.app-grid'))", 15000));
    assertStep("PACKAGED_PASS33_RAIL", await waitForEval(cdp, "Boolean(document.querySelector('.app-rail') && document.querySelector('.workspace-context-tabs'))", 5000));
    await capture(cdp, "packaged-pass33-home.png");
    ...
    `));
    assertStep("PACKAGED_PASS33_ENVIRONMENT_PANEL", await waitForEval(cdp, "Boolean(document.querySelector('.bottom-work-panel'))", 5000));
    await capture(cdp, "packaged-pass33-environment.png");
    ...
    `));
    assertStep("PACKAGED_PASS33_CHANGES_PANEL", await waitForEval(cdp, "Boolean(document.querySelector('.git-status-preview'))", 5000));
    await capture(cdp, "packaged-pass33-changes.png");

    console.log("PACKAGED_PASS33_DONE");
  } finally {
    if (cdp) cdp.close();
    child.kill();
  }
...
main().catch((error) => {
  console.error("PACKAGED_PASS33_FAILED", error.stack || error);
  process.exit(1);
});
