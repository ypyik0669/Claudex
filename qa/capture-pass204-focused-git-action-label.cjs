...
const USER_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass204-data-"));
const GIT_PROJECT_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass204-git-"));
const FAKE_BIN_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass204-bin-"));
const FAKE_CLAUDE = path.join(FAKE_BIN_DIR, "claude.cmd");
const DATA_FILE = path.join(USER_DATA_DIR, "desktop-data.json");
const TARGET_FILE = "pass204-changes.txt";
const FAILURE_RUN_ID = "pass204-git-failed-run";
const SUCCESS_RUN_ID = "pass204-git-success-run";
...
function writeFakeClaude() {
  ...
  fs.writeFileSync(
    ...
    [
      "@echo off",
      "if \"%1\"==\"--version\" (echo claude fake pass204& exit /b 0)",
...
function setupGitProject() {
  ...
  fs.writeFileSync(path.join(GIT_PROJECT_DIR, TARGET_FILE), "pass204 baseline\n", "utf8");
  runGit(["init"]);
  ...
  fs.writeFileSync(path.join(GIT_PROJECT_DIR, TARGET_FILE), "pass204 baseline\npass204 dirty changes evidence\n", "utf8");
}

function writeInitialStore() {
  writeFakeClaude();
  setupGitProject();
  const project = { name: "pass204-git-project", path: GIT_PROJECT_DIR };
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      sessions: [
        {
          id: "pass204-session",
          title: "Pass204 focused git action label",
      ...
      commandRuns: [
        ...
        },
        {
          ...
          code: 128,
          durationMs: 203,
          stdout: "pass204 git stdout before failure",
      ...
      runEvents: [
        {
          ...
          type: "git-command",
          status: "ok",
          title: "Git: pass204 status succeeded",
          detail: `pass204 latest successful git action · git status --short`,
          commandLine: "git status --short",
          cwd: GIT_PROJECT_DIR,
          project,
          sessionId: "pass204-session",
          code: 0,
          durationMs: 80,
        ...
        },
        {
          ...
          type: "git-command",
          status: "error",
          title: "Git: pass204 stage failed",
          detail: `pass204 latest failed git action detail · git add missing-${TARGET_FILE}`,
          commandLine: `git add missing-${TARGET_FILE}`,
          cwd: GIT_PROJECT_DIR,
          project,
          sessionId: "pass204-session",
...
async function runTest() {
  ...
  if (!win) throw new Error("PASS204_FAILED_NO_WINDOW");
  win.setBounds({ x: 0, y: 0, width: 1500, height: 980 });
  await wait(700);

  assertStep("PASS204_READY", await waitFor(win, "Boolean(document.querySelector('.app-grid') && window.claudexDesktop)", 15000));
  assertStep("PASS204_GIT_STATUS_BUCKET_STATE_READY", await waitFor(win, `
    (async function() {
  ...
  assertStep("PASS204_OPEN_CHANGES_DEFAULT_RECENT_LABEL", await openChanges(win));
  assertStep("PASS204_DEFAULT_CHANGES_LABEL_RECENT", await waitFor(win, `
    (function() {
      ...
      return /\\u53d8\\u66f4/.test(active) &&
        ...
        /Git: pass204 status succeeded/.test(latest) &&
        /git status --short/.test(latest);
    })();
  `, 10000));

  assertStep("PASS204_OPEN_PALETTE_FAILED_GIT_ACTION", await openPaletteAndQuery(win, "failed git pass204"));
  assertStep("PASS204_PALETTE_FAILED_GIT_ACTION_COMMAND", await waitFor(win, `
    (function() {
      const button = [...document.querySelectorAll('.command-modal .command-list button')]
      ...
      return Boolean(button &&
        ...
        /Git: pass204 stage failed/.test(text) &&
        /git add missing-${TARGET_FILE}/.test(text) &&
        /128/.test(text));
    })();
  `, 8000));

  assertStep("PASS204_CLICK_FAILED_GIT_ACTION_COMMAND", await clickCommandById(win, `git-latest-failed-action:${FAILURE_RUN_ID}`));
  assertStep("PASS204_FAILED_COMMAND_OPENS_CHANGES_EVIDENCE", await waitFor(win, `
    (function() {
      const active = document.querySelector('.workspace-context-button.active')?.textContent || document.querySelector('.bottom-panel-tabs button.active')?.textContent || '';
      ...
      return /\\u53d8\\u66f4/.test(active) &&
        /\\u805a\\u7126 Git \\u64cd\\u4f5c/.test(latest) &&
        !/\\u6700\\u8fd1 Git \\u64cd\\u4f5c/.test(latest) &&
        /Git: pass204 stage failed/.test(latest) &&
        /pass204 latest failed git action detail/.test(latest) &&
        /git add missing-${TARGET_FILE}/.test(latest) &&
        /pass204 git stdout before failure/.test(latest) &&
        /pathspec|did not match|fatal/i.test(latest);
    })();
  `, 10000));

  assertStep("PASS204_OPEN_FAILED_GIT_TIMELINE", await win.webContents.executeJavaScript(`
    (function() {
  ...
  `));
  assertStep("PASS204_TIMELINE_FOCUSED_ON_FAILED_GIT_ACTION", await waitFor(win, `
    (function() {
      const active = document.querySelector('.workspace-context-button.active')?.textContent || document.querySelector('.bottom-panel-tabs button.active')?.textContent || '';
      ...
      return Boolean(/\\u8f93\\u51fa|Outputs/i.test(active) &&
        ...
        /Git: pass204 stage failed/.test(text) &&
        /git add missing-${TARGET_FILE}/.test(text) &&
        /pass204 git stdout before failure/.test(text) &&
        /pathspec/.test(text) &&
        panel.querySelector('[data-run-event-type="git-command"]'));
    })();
  `, 10000));

  assertStep("PASS204_OPEN_PALETTE_SUCCESSFUL_GIT_ACTION", await openPaletteAndQuery(win, "successful git pass204"));
  assertStep("PASS204_PALETTE_SUCCESSFUL_GIT_ACTION_COMMAND", await waitFor(win, `
    (function() {
      const button = [...document.querySelectorAll('.command-modal .command-list button')]
      ...
      return Boolean(button &&
        ...
        /\\u67e5\\u770b\\u53d8\\u66f4\\u8bc1\\u636e/.test(text) &&
        /Git: pass204 status succeeded/.test(text) &&
        /git status --short/.test(text) &&
        /0/.test(text));
    })();
  `, 8000));

  assertStep("PASS204_CLICK_SUCCESSFUL_GIT_ACTION_COMMAND", await clickCommandById(win, `git-latest-successful-action:${SUCCESS_RUN_ID}`));
  assertStep("PASS204_SUCCESSFUL_COMMAND_OPENS_CHANGES_EVIDENCE", await waitFor(win, `
    (function() {
      const active = document.querySelector('.workspace-context-button.active')?.textContent || document.querySelector('.bottom-panel-tabs button.active')?.textContent || '';
      const latest = document.querySelector('.git-latest-action.ok')?.textContent || '';
      return /\\u53d8\\u66f4/.test(active) &&
        ...
        !/\\u6700\\u8fd1 Git \\u64cd\\u4f5c/.test(latest) &&
        /Git: pass204 status succeeded/.test(latest) &&
        /pass204 latest successful git action/.test(latest) &&
        /git status --short/.test(latest) &&
        /${TARGET_FILE}/.test(latest);
    })();
  `, 10000));

  assertStep("PASS204_OPEN_PALETTE_LATEST_GIT_ACTION", await openPaletteAndQuery(win, "latest git action pass204"));
  assertStep("PASS204_PALETTE_LATEST_GIT_ACTION_STILL_POINTS_TO_SUCCESS", await waitFor(win, `
    (function() {
      const button = [...document.querySelectorAll('.command-modal .command-list button')]
  ...
  console.log("PASS204_FOCUSED_GIT_ACTION_LABEL_DONE");
  cleanup();
  app.exit(0);
}
...
app.whenReady().then(runTest).catch((error) => {
  console.error("PASS204_FAILED", error?.stack || error);
  Promise.resolve()
    .then(async () => {
      const win = BrowserWindow.getAllWindows()[0];
      if (!win) return;
      ...
      console.error("PASS204_DEBUG", JSON.stringify(debug, null, 2).slice(0, 8000));
    })
    .finally(() => {
      cleanup();
      app.exit(1);
