...
const EXE_PATH = path.join(PROJECT_PATH, "release-pass30", "win-unpacked", "Claudex.exe");
const PORT = 12430 + Math.floor(Math.random() * 500);
...
async function main() {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass30-packaged-"));
  ...
  try {
    ...
    assertStep("PACKAGED_PASS30_READY", await waitForEval(cdp, "Boolean(document.querySelector('.app-grid'))", 15000));
    assertStep("PACKAGED_PASS30_BOTTOM_PANEL_CLICK", await evalInPage(cdp, `
      (function() {
    ...
    `));
    assertStep("PACKAGED_PASS30_BOTTOM_PANEL", await waitForEval(cdp, "Boolean(document.querySelector('.bottom-work-panel'))", 5000));
    assertStep("PACKAGED_PASS30_SETTINGS_SURFACE_CLICK", await evalInPage(cdp, `
      (function() {
    ...
    `));
    assertStep("PACKAGED_PASS30_SETTINGS_SURFACE", await waitForEval(cdp, "Boolean(document.querySelector('.settings-surface'))", 5000));
    await capture(cdp, "packaged-pass30-settings.png");
    console.log("PACKAGED_PASS30_DONE");
  } finally {
    if (cdp) cdp.close();
    child.kill();
  }
...
main().catch((error) => {
  console.error("PACKAGED_PASS30_FAILED", error.stack || error);
  process.exit(1);
});
