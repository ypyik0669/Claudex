...
const USER_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass206-data-"));
const GIT_PROJECT_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass206-git-"));
const FAKE_BIN_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "claudex-pass206-bin-"));
const FAKE_CLAUDE = path.join(FAKE_BIN_DIR, "claude.cmd");
const DATA_FILE = path.join(USER_DATA_DIR, "desktop-data.json");
const TARGET_FILE = "pass206-changes.txt";
const FAILURE_RUN_ID = "pass206-git-failed-run";
const SUCCESS_RUN_ID = "pass206-git-success-run";
...
function writeFakeClaude() {
  ...
  fs.writeFileSync(
    ...
    [
      "@echo off",
      "if \"%1\"==\"--version\" (echo claude fake pass206& exit /b 0)",
...
function setupGitProject() {
  ...
  fs.writeFileSync(path.join(GIT_PROJECT_DIR, TARGET_FILE), "pass206 baseline\n", "utf8");
  runGit(["init"]);
  ...
  fs.writeFileSync(path.join(GIT_PROJECT_DIR, TARGET_FILE), "pass206 baseline\npass206 dirty changes evidence\n", "utf8");
}

function writeInitialStore() {
  writeFakeClaude();
  setupGitProject();
  const project = { name: "pass206-git-project", path: GIT_PROJECT_DIR };
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      sessions: [
        {
          id: "pass206-session",
          title: "Pass206 clear git action focus",
      ...
      commandRuns: [
        ...
        },
        {
          ...
          code: 128,
          durationMs: 203,
          stdout: "pass206 git stdout before failure",
      ...
      runEvents: [
        {
          ...
          type: "git-command",
          status: "ok",
          title: "Git: pass206 status succeeded",
          detail: `pass206 latest successful git action Â· git status --short`,
          commandLine: "git status --short",
          cwd: GIT_PROJECT_DIR,
          project,
          sessionId: "pass206-session",
          code: 0,
          durationMs: 80,
        ...
        },
        {
          ...
          type: "git-command",
          status: "error",
          title: "Git: pass206 stage failed",
          detail: `pass206 latest failed git action detail Â· git add missing-${TARGET_FILE}`,
          commandLine: `git add missing-${TARGET_FILE}`,
          cwd: GIT_PROJECT_DIR,
          project,
          sessionId: "pass206-session",
...
async function runTest() {
  ...
  if (!win) throw new Error("PASS206_FAILED_NO_WINDOW");
  win.setBounds({ x: 0, y: 0, width: 1500, height: 980 });
  await wait(700);

  assertStep("PASS206_READY", await waitFor(win, "Boolean(document.querySelector('.app-grid') && window.claudexDesktop)", 15000));
  assertStep("PASS206_GIT_STATUS_BUCKET_STATE_READY", await waitFor(win, `
    (async function() {
  ...
  assertStep("PASS206_OPEN_CHANGES_DEFAULT_RECENT_LABEL", await openChanges(win));
  assertStep("PASS206_DEFAULT_CHANGES_LABEL_RECENT", await waitFor(win, `
    (function() {
      ...
      return /\\u53d8\\u66f4/.test(active) &&
        /\\u6700\\u8fd1 Git \\u64cd\\u4f5c/.test(latest) &&
        !/\\u805a\\u7126 Git \\u64cd\\u4f5c/.test(latest) &&
        /Git: pass206 status succeeded/.test(latest) &&
        /git status --short/.test(latest);
    })();
  `, 10000));

  assertStep("PASS206_OPEN_PALETTE_FAILED_GIT_ACTION", await openPaletteAndQuery(win, "failed git pass206"));
  assertStep("PASS206_PALETTE_FAILED_GIT_ACTION_COMMAND", await waitFor(win, `
    (function() {
      const button = [...document.querySelectorAll('.command-modal .command-list button')]
      ...
      return Boolean(button &&
        ...
        /Git: pass206 stage failed/.test(text) &&
        /git add missing-${TARGET_FILE}/.test(text) &&
        /128/.test(text));
    })();
  `, 8000));

  assertStep("PASS206_CLICK_FAILED_GIT_ACTION_COMMAND", await clickCommandById(win, `git-latest-failed-action:${FAILURE_RUN_ID}`));
  assertStep("PASS206_FAILED_COMMAND_OPENS_CHANGES_EVIDENCE", await waitFor(win, `
    (function() {
      const active = document.querySelector('.workspace-context-button.active')?.textContent || document.querySelector('.bottom-panel-tabs button.active')?.textContent || '';
      ...
      return /\\u53d8\\u66f4/.test(active) &&
        /\\u805a\\u7126 Git \\u64cd\\u4f5c/.test(latest) &&
        !/\\u6700\\u8fd1 Git \\u64cd\\u4f5c/.test(latest) &&
        /Git: pass206 stage failed/.test(latest) &&
        /pass206 latest failed git action detail/.test(latest) &&
        /git add missing-${TARGET_FILE}/.test(latest) &&
        /pass206 git stdout before failure/.test(latest) &&
        /pathspec|did not match|fatal/i.test(latest);
    })();
  `, 10000));

  assertStep("PASS206_OPEN_PALETTE_CLEAR_GIT_FOCUS", await openPaletteAndQuery(win, "回到最近 pass206"));
  assertStep("PASS206_PALETTE_CLEAR_GIT_FOCUS_COMMAND", await waitFor(win, `
    (function() {
      const button = [...document.querySelectorAll('.command-modal .command-list button')]
        .find((candidate) => (candidate.getAttribute('data-command-id') || '') === 'git-clear-action-focus:${FAILURE_RUN_ID}');
      const text = button?.textContent || '';
      return Boolean(button &&
        button.getAttribute('data-command-target') === 'changes' &&
        /\\u56de\\u5230\\u6700\\u8fd1/.test(text) &&
  ...
  `, 8000));
  assertStep("PASS206_CLICK_CLEAR_GIT_FOCUS_COMMAND", await clickCommandById(win, `git-clear-action-focus:${FAILURE_RUN_ID}`));
  assertStep("PASS206_COMMAND_CLEAR_FOCUS_RETURNS_TO_RECENT_SUCCESS", await waitFor(win, `
    (function() {
      const active = document.querySelector('.workspace-context-button.active')?.textContent || document.querySelector('.bottom-panel-tabs button.active')?.textContent || '';
      const latest = document.querySelector('.git-latest-action.ok')?.textContent || '';
      return /\\u53d8\\u66f4/.test(active) &&
        /\\u6700\\u8fd1 Git \\u64cd\\u4f5c/.test(latest) &&
        !/\\u805a\\u7126 Git \\u64cd\\u4f5c/.test(latest) &&
        !document.querySelector('.git-latest-action button[data-git-action="clear-focus"]') &&
        /Git: pass206 status succeeded/.test(latest) &&
        /git status --short/.test(latest) &&
  ...
  assertStep("PASS206_OPEN_PALETTE_SUCCESSFUL_GIT_ACTION", await openPaletteAndQuery(win, "successful git pass206"));
  assertStep("PASS206_PALETTE_SUCCESSFUL_GIT_ACTION_COMMAND", await waitFor(win, `
    (function() {
      const button = [...document.querySelectorAll('.command-modal .command-list button')]
      ...
      return Boolean(button &&
        ...
        /\\u67e5\\u770b\\u53d8\\u66f4\\u8bc1\\u636e/.test(text) &&
        /Git: pass206 status succeeded/.test(text) &&
        /git status --short/.test(text) &&
        /0/.test(text));
    })();
  `, 8000));

  assertStep("PASS206_CLICK_SUCCESSFUL_GIT_ACTION_COMMAND", await clickCommandById(win, `git-latest-successful-action:${SUCCESS_RUN_ID}`));
  assertStep("PASS206_SUCCESSFUL_COMMAND_OPENS_CHANGES_EVIDENCE", await waitFor(win, `
    (function() {
      const active = document.querySelector('.workspace-context-button.active')?.textContent || document.querySelector('.bottom-panel-tabs button.active')?.textContent || '';
      const latest = document.querySelector('.git-latest-action.ok')?.textContent || '';
      return /\\u53d8\\u66f4/.test(active) &&
        ...
        !/\\u6700\\u8fd1 Git \\u64cd\\u4f5c/.test(latest) &&
        /Git: pass206 status succeeded/.test(latest) &&
        /pass206 latest successful git action/.test(latest) &&
        /git status --short/.test(latest) &&
        /${TARGET_FILE}/.test(latest);
    })();
  `, 10000));

  assertStep("PASS206_OPEN_PALETTE_LATEST_GIT_ACTION", await openPaletteAndQuery(win, "latest git action pass206"));
  assertStep("PASS206_PALETTE_LATEST_GIT_ACTION_STILL_POINTS_TO_SUCCESS", await waitFor(win, `
    (function() {
      const button = [...document.querySelectorAll('.command-modal .command-list button')]
  ...
  console.log("PASS206_CLEAR_GIT_ACTION_FOCUS_DONE");
  cleanup();
  app.exit(0);
}
...
app.whenReady().then(runTest).catch((error) => {
  console.error("PASS206_FAILED", error?.stack || error);
  Promise.resolve()
    .then(async () => {
      const win = BrowserWindow.getAllWindows()[0];
      if (!win) return;
      ...
      console.error("PASS206_DEBUG", JSON.stringify(debug, null, 2).slice(0, 8000));
    })
    .finally(() => {
      cleanup();
      app.exit(1);
